...
/**
 * Implements a Task that sets a date header on an {@link javax.servlet.http.HttpServletResponse
...
public class ServletSetDateHeaderTask implements Task {
	...
	public static final Reagent SOURCE = new SimpleReagent("SOURCE", "@source", ReagentType.PHRASE, HttpServletResponse.class,
								"Optional source response to set date the header on.  If not specified, the value of "
	...
	public static final Reagent HEADER_VALUE = new SimpleReagent("HEADER_VALUE", "@value", ReagentType.PHRASE, String.class,
								"The (long) value of the date (time in ms from the epoch).");
	...
	public void perform(TaskRequest req, TaskResponse res) {
		...
		// Send the header
		sres.setDateHeader(headerName, Long.valueOf(headerValue));
	}
}
